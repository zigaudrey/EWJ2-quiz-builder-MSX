...
if len(txt_file) != 0:
    ...
    button_bit = [b'\x01\x0B\x18', b'\x01\x0B\x19', b'\x01\x0B\x1A']
    ...
    for n in range(0, len(quiz_data), 5):
        ...
        new_line += struct.pack("B", 3 + floor((30 - len_list[0]) / 4))
        ...
        for x in range(1,len(quiz_data[n])):
            if quiz_data[n][x] == "%":
                new_line += b'\x07\x01'
                new_line +=  struct.pack("B", 3 + floor((30 - len_list[xl]) / 4))
                xl += 1
            else:
    ...
    if len(data_bin) > 8286:
        print("Quiz Data Bin exceed 8286 bytes.")
    else:
        if len(data_bin) < 8286:
